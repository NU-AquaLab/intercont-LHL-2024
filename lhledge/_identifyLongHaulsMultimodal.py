...
class _longHaulDetectorMultimodal(object):
    """docstring for longHaulDetector"""
    def __init__(self):
        super(_longHaulDetectorMultimodal, self).__init__()
        ...
        if df.shape[0] > 2:
            ...
            if idx.size > 0:
                ...
                for i in idx:
                    ...
                    try:
                        ns_q_ttl = int(df["icmp_q_ttl"][i - 1])
                    except:
                        ns_q_ttl = 0

                    try:
                        fs_q_ttl = int(df["icmp_q_ttl"][i])
                    except:
                        fs_q_ttl = 0
                    ...
                    long_haul_link = {
                        ...
                        "far_side_rdns": rdns1,
                        "near_side_q_ttl": ns_q_ttl,
                        "far_side_q_ttl": fs_q_ttl,
                        "near_side_reply_ttl": int(df["reply_ttl"][i - 1]),
                        "far_side_reply_ttl": int(df["reply_ttl"][i]),
